...
class DeveloperController {
  async index({ request }) {
    ...
    if (data.search) query.orWhere("name", "LIKE", `%${data.search}%`);
    if (data.id) query.orWhere("id", "=", data.id);
  ...
  async store({ request, response }) {
    try {
      ...
      return response.send({
        message:
          "Ocorreu um erro ao inserir os dados de desenvolvedor, por favor verifique!",
      });
    }
  }
  ...
  async show({ params, response }) {
    try {
      ...
      return response.send({
        message:
          "Ocorreu um erro ao exibir os dados de desenvolvedor, por favor verifique!",
      });
    }
  }
  ...
  async update({ params, request, response }) {
    try {
      ...
      return response.send({
        message:
          "Ocorreu um erro ao atualizar os dados de desenvolvedor, por favor verifique!",
      });
    }
  }
  async destroy({ params, response }) {
    try {
      ...
      return response.send({
        message:
          "Ocorreu um erro ao excluir os dados de desenvolvedor, por favor verifique!",
      });
    }
  }
}
